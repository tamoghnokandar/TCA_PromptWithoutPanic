...
if [ "$arch" == "ViT-B/16" ]; then
	output_file="Training_Runs/output_alpha_${alpha}_beta_${beta}_arch_VIT_num_attributes_${num_attributes}_dataset_${testsets}.txt"
else
	output_file="Training_Runs/output_alpha_${alpha}_beta_${beta}_arch_RN_num_attributes_${num_attributes}_dataset_${testsets}.txt"
fi
